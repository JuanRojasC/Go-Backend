...
import (
	...
	"github.com/JuanDRojasC/C18-GoNoSQL-TM/go-web/pkg/web"
...
func (ph *ProductHandler) GetAll() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		context, cancel := ContextTimeout(5)
		defer cancel()
		all, err := ph.service.GetAll(context)
		if err != nil {
...
func (ph *ProductHandler) GetOne() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		context, cancel := ContextTimeout(5)
		defer cancel()
		p, err := ph.service.GetOne(context, id)
		if err != nil {
			ctx.JSON(http.StatusNotFound, web.NewReponse(http.StatusNotFound, nil, err))
			return
		}
...
func (ph *ProductHandler) SaveProduct() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		context, cancel := ContextTimeout(5)
		defer cancel()
		p, err := ph.service.Save(context, req.Name, req.Color, req.Price, *req.Stock, req.Code, *req.Published)
		if err != nil {
			ctx.JSON(http.StatusNotFound, web.NewReponse(http.StatusNotFound, nil, err))
			return
		}
...
func (ph *ProductHandler) UpdateProduct() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		context, cancel := ContextTimeout(5)
		defer cancel()
		p, err := ph.service.Update(context, id, req.Name, req.Color, req.Price, *req.Stock, req.Code, *req.Published)
		if err != nil {
			ctx.JSON(http.StatusNotFound, web.NewReponse(http.StatusNotFound, nil, err))
			return
		}
...
func (ph *ProductHandler) Delete() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		context, cancel := ContextTimeout(5)
		defer cancel()
		if err := ph.service.Delete(context, id); err != nil {
			ctx.JSON(http.StatusNotFound, web.NewReponse(http.StatusNotFound, nil, err))
			return
		}
...
func ContextTimeout(seconds int) (context.Context, context.CancelFunc) {
