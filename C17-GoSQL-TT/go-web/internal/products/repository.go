...
const (
	...
	getID         string = "SELECT " + string(ID) + " FROM products WHERE id = ?"
	...
	deleteByID    string = "DELETE FROM products WHERE id = ?"
)
...
func (r *repository) CheckExistence(id int) error {
	rows, err := r.db.Query(getByID, id)
	if err != nil {
		return ErrInternalServer
	}
